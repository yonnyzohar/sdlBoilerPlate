...
void Polygon::fillTriangle()
{
   ...
   float p2z = p2.z;
     ...
     if (p0y < p1y) {
         ...
         float leftYDiff = (p1y - p0y);
         //slope from top to first side
         float slope1 = (p1x - p0x) / leftYDiff;
         //slope from top to second side
         float slope2 = (p2x - p0x) / (p2y - p0y);
         float triangleHeight = p1y - p0y;
         for (int i = 0; i <= triangleHeight; i++) {
             int startX = p0x + i * slope1;
             int endX = p0x + i * slope2;
             int _y = p0y + i;

             //u start and v start
             float us  = p0u + (_y - p0y) / leftYDiff * (p1u - p0u);
             float vs  = p0v + (_y - p0y) / leftYDiff * (p1v - p0v);
             //float ws  = p0w + (_y - p0y) / leftYDiff * (p1w - p0w);

             //u end and v end
             float ue = p0u + (_y - p0y) / (p2y - p0y) * (p2u - p0u);
             float ve = p0v + (_y - p0y) / (p2y - p0y) * (p2v - p0v);
             //float we = p0w + (_y - p0y) / (p2y - p0y) * (p2w - p0w);

             //z buffer start and end
             float zs = p0z + (_y - p0y) / leftYDiff * (p1z - p0z);
             float ze = p0z + (_y - p0y) / (p2y - p0y) * (p2z - p0z);
     ...
     if (p1y < p2y) {
         //slope from top to first side
         float slope1 = (p2x - p1x) / (p2y - p1y);
         //slope from top to second side
         float slope2 = (p2x - p0x) / (p2y - p0y);
         float sx = p2x - (p2y - p1y) * slope2;

         float triangleHeight = p2y - p1y;

         for (int i = 0; i <= triangleHeight; i++) {
             int startX = p1x + i * slope1;
             int endX = sx + i * slope2;
             int _y = p1y + i;

             //u start and v start
             float us = p1u + (_y - p1y) / (p2y - p1y) * (p2u - p1u);
             float vs = p1v + (_y - p1y) / (p2y - p1y) * (p2v - p1v);

             //float ws = p1w + (_y - p1y) / (p2y - p1y) * (p2w - p1w);
             ...
             //z buffer start and end
             float zs = p1z + (_y - p1y) / (p2y - p1y) * (p2z - p1z);
             float ze = p0z + (_y - p0y) / (p2y - p0y) * (p2z - p0z);
